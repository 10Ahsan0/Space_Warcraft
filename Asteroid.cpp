...
Asteroid::Asteroid(sf::Vector2f direction, sf::Vector2f position)
	: Entity(position, 0), direction(direction), array(sf::TrianglesFan, 12), life() {
...
void Asteroid::update(float deltaTime) {
	...
	//position += (ASTEROID_SPEED + life * ASTEROID_ACCELERATION_RATE) * direction * deltaTime;
	if (life <= 4.0f)
		position += (ASTEROID_SPEED + life * ASTEROID_ACCELERATION_RATE) * direction * deltaTime;
	else if (life > 4.0f && life < 10.0f)
		position += (ASTEROID_SPEED * 1.68f + life * 11.0f) * direction * deltaTime;
	else
		position += (ASTEROID_SPEED * 3.20f) * direction * deltaTime;
...
sf::Vector2f Asteroid::getRandomPosition() {
	...
	std::uniform_real_distribution<float> yAxis(ASTEROID_H / 2.0f,
		SCREEN_HEIGHT/2 - ASTEROID_H / 2.0f)/*SCREEN_HEIGHT- ASTEROID_H / 2.0f*/;
